...
The workflow integrates established bioinformatics tools for alignment, primer trimming, consensus generation, and lineage assignment, while exposing only a limited set of operationally relevant parameters. Depth-aware filtering and post-alignment primer trimming are implemented as core workflow behaviors to reduce amplification artifacts and improve reproducibility of consensus genome generation from tiled amplicon sequencing data [@Bull:2020]. 
...
Discrepancies between CECRET and nf-core/viralrecon were primarily driven by differences in consensus frequency thresholds and pipeline quality gates. nf-core/viralrecon enforces strict mapping and coverage thresholds that caused it to abort processing on two low-coverage samples (<100,000 reads). In contrast, CECRET is designed to complete execution on low-depth samples by outputting consensus genomes heavily masked with Ns at low-coverage positions. Furthermore, subtle SNP differences (0–7 SNPs) occurred at sites with minor intra-host variation due to differing consensus allele frequency cutoffs (0.6 in CECRET vs. 0.75 in nf-core/viralrecon), which subsequently influenced Pangolin lineage assignments in high-N samples.
